...
export default function LoginPage() {
  ...
  // 2. Parse query parameters on load to auto-toggle register mode
  useEffect(() => {
  ...
  // 3. Reactive redirect observer - triggers when user context changes
  useEffect(() => {
    if (user && user.email) {
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (isRegisterMode) {
      if (!name) {
        showToast("Vennligst oppgi navn");
        return;
      }
      await registerWithEmail(email, password, name, 'member');
    } else {
      ...
      >
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-on-surface-variant max-w-md mx-auto">
            {isRegisterMode 
              ? "Opprett en brukerkonto for å søke opptak, betale for kurs og få tilgang til studieportalen." 
        ...
        <div className="space-y-2 mb-6">
          ...
          <p className="text-[10px] text-center text-on-surface-variant/70 font-medium">
            {isRegisterMode 
              ? "Konto opprettes automatisk hvis du ikke har en fra før." 
              : "Vil også registrere en ny konto om du logger inn for første gang."}
          </p>
        </div>
        ...
        <div className="flex items-center gap-4 my-4">
          ...
          <span className="text-xs text-[#72787e] uppercase tracking-wider font-bold">
            {isRegisterMode ? "Eller fyll inn info" : "Eller med e-post"}
          </span>
          <div className="flex-1 h-[1px] bg-outline-variant/50" />
        </div>
        ...
        <div>
          {/* Login Method Toggle: Password vs Passwordless (Only show in login mode) */}
          {!isRegisterMode && (
              ...
              <div className="form-field-stable">
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full bg-background border border-outline-variant rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-[#561291] focus:outline-none transition-all"
                    required
                  />
                </div>
              </div>
            ...
            {(isRegisterMode || loginMethod === 'password') && (
              <div className="form-field-stable">
                <div className="flex justify-between items-center mb-2">
                  <label className="block text-xs font-bold text-[#41474d] uppercase tracking-wider">
                    {isRegisterMode ? "Velg passord (min. 6 tegn)" : "Passord"}
                  </label>
                  {!isRegisterMode && (
                    <button 
                      type="button" 
          ...
          <div className="text-center mt-6 pt-4 border-t border-outline-variant/30">
            <button
              ...
              className="text-xs text-[#3c096c] hover:underline font-bold uppercase tracking-wider"
            >
              {isRegisterMode 
                ? 'Har du allerede en brukerkonto? Logg inn her' 
                : 'Har du ingen brukerkonto? Opprett en her'}
            </button>
          </div>
        </div>
