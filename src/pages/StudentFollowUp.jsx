...
export default function StudentFollowUp() {
  ...
  return (
    <div className="w-full px-4 sm:px-6 md:px-12 py-6 md:py-12 flex flex-col gap-6 md:gap-8 font-sans">
      ...
      {/* Filters/Summary */}
      <div className="bg-white border border-outline-variant/30 rounded-xl p-4 sm:p-5 shadow-sm flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {students.map(stud => {
          ...
          return (
            <div 
              key={stud.id} 
              className="bg-white border border-outline-variant/30 rounded-xl p-5 sm:p-6 shadow-sm flex flex-col justify-between gap-6 hover:border-primary transition-all duration-300 relative overflow-hidden group hover:shadow-md"
            >
        ...
        {selectedStudent && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-slate-900/50 backdrop-blur-sm">
            <motion.div 
              ...
              className="bg-white border border-outline-variant/30 rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden"
              style={{
            ...
            >
              ...
              <div className="p-4 sm:p-6 bg-primary text-white flex justify-between items-center shrink-0">
                <div className="space-y-1 min-w-0 mr-4">
                  ...
                  <p className="text-xs text-white/90 truncate">Mottaker: {selectedStudent.name} ({selectedStudent.courseName})</p>
                </div>
                <button 
                  onClick={() => setSelectedStudent(null)}
                  className="p-1.5 hover:bg-white/10 rounded-full text-white/90 transition-colors shrink-0 active:scale-95"
                >
                  <X size={20} />
                </button>
              </div>
              ...
              {/* Form body */}
              <form onSubmit={handleSend} className="p-4 sm:p-6 space-y-4 sm:space-y-6 overflow-y-auto flex-grow">
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4 font-sans">
                    <button
                      ...
                      className={`py-2.5 sm:py-3 px-4 rounded-lg border font-bold text-xs uppercase tracking-wider text-center transition-all duration-300 ${
                ...
                {/* Submitting Buttons */}
                <div className="flex flex-col-reverse sm:flex-row gap-3 sm:gap-4 justify-end pt-4 border-t border-outline-variant/30 shrink-0">
                  <button
                    type="button"
                    onClick={() => setSelectedStudent(null)}
