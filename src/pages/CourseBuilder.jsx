...
// ─── Static media assets (shared with MediaLibrary) ───────────────────────
const MEDIA_ASSETS = [
...
const PROGRESSION_RULES = [
  { id: 'sequential', name: 'Sekvensiell', desc: 'Elever fullfører én modul før de låser opp neste.' },
...
function InlineField({ value, onSave, placeholder = '', className = '', inputClassName = '' }) {
  ...
  if (editing) {
    return (
      <div className="flex items-center gap-1.5 w-full">
        ...
        />
        <button onClick={commit} className="text-emerald-600 hover:text-emerald-700 p-1 shrink-0" title="Lagre"><Save size={14} /></button>
        <button onClick={cancel} className="text-outline hover:text-red-500 p-1 shrink-0" title="Avbryt"><XCircle size={14} /></button>
      </div>
    );
  }
  ...
  return (
    <button
      ...
      className={`group flex items-center gap-1.5 text-left w-full hover:text-primary transition-colors ${className}`}
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2.5 p-3.5 bg-slate-50/70">
        ...
        <span className="text-outline cursor-grab shrink-0 touch-none select-none">
        ...
        <div className="flex-grow min-w-0">
          ...
          />
          {linked.length > 0 && (
            <p className="text-[9px] font-bold text-[#c5a059] mt-0.5 truncate">
        ...
        <button
          ...
          className={`flex items-center gap-1 text-[10px] font-bold px-2.5 py-1 rounded-full border transition-all shrink-0 ${
            mod.completed
        ...
        >
          {mod.completed
            ? <><CheckCircle2 size={11} /> Publisert</>
            : <><Circle size={11} /> Utkast</>}
        </button>
        ...
        <div className="flex flex-col gap-0.5 shrink-0">
          <button
            disabled={index === 0}
            onClick={() => reorderModule(courseId, mod.id, 'up')}
            className="p-1 hover:bg-primary/10 rounded text-outline hover:text-primary transition-colors disabled:opacity-25 disabled:cursor-not-allowed"
            title="Flytt opp"
          >
            <ArrowUp size={12} />
          </button>
          <button
            ...
            onClick={() => reorderModule(courseId, mod.id, 'down')}
            className="p-1 hover:bg-primary/10 rounded text-outline hover:text-primary transition-colors disabled:opacity-25 disabled:cursor-not-allowed"
            title="Flytt ned"
          >
            <ArrowDown size={12} />
      ...
      <AnimatePresence>
        {expanded && (
          ...
          >
            <div className="border-t border-outline-variant/20 bg-white px-5 py-4 flex flex-col sm:flex-row gap-4 items-start sm:items-center">

              {/* Linked asset chips */}
              <div className="flex flex-wrap gap-2 flex-grow min-w-0">
                {linked.length === 0 ? (
                  <span className="text-[11px] text-outline-variant font-semibold italic">Ingen ressurser koblet til denne modulen ennå.</span>
                ) : (
                  linked.map(assetId => {
              ...
              <div className="flex items-center gap-2.5 shrink-0">
                ...
                {confirmDelete ? (
                  <div className="flex items-center gap-1.5 bg-red-50 border border-red-200 px-3 py-1.5 rounded-lg">
                    ...
                    <button
                      onClick={() => deleteModule(courseId, mod.id)}
                      className="text-[10px] font-bold text-white bg-red-500 hover:bg-red-600 px-2 py-0.5 rounded transition-colors"
                    >Ja</button>
                    <button
                      onClick={() => setConfirmDelete(false)}
                      className="text-[10px] font-bold text-outline hover:text-primary"
                    >Nei</button>
                  </div>
                ) : (
                  <button
                    onClick={() => setConfirmDelete(true)}
                    className="p-2 hover:bg-red-50 border border-transparent hover:border-red-200 rounded-lg text-outline hover:text-red-500 transition-colors"
                    title="Slett modul"
                  >
                    <Trash2 size={15} />
                  </button>
...
export default function CourseBuilder() {
  ...
  // Course header inline edit state
  ...
  const cancelCourseEdit = () => setIsEditingCourse(false);
    ...
    <>
      <div className="w-full px-4 sm:px-6 md:px-10 py-6 md:py-10 flex flex-col gap-6 md:gap-8 font-sans overflow-x-hidden">
        ...
        <div className="flex flex-col xl:flex-row gap-6 md:gap-8">
          ...
          {/* ─── LEFT: Course selector + progression ─── */}
          <div className="w-full xl:w-[320px] flex flex-col gap-5 shrink-0">
            ...
            <div className="bg-white border border-outline-variant/30 rounded-xl p-5 shadow-sm">
              <h2 className="font-serif text-lg font-bold text-primary mb-1 flex items-center gap-2">
                <Layers size={18} className="text-[#c5a059] shrink-0" />
                Studielinjer
              </h2>
                ...
                {courses.map(course => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left p-4 rounded-xl border-2 transition-all duration-150 active:scale-[0.99] group ${
                        isActive
                          ? 'bg-primary border-primary text-white shadow-sm'
                          : 'bg-white border-outline-variant/25 hover:border-primary/40 hover:bg-slate-50 text-on-surface-variant'
                      }`}
                    >
                      <div className="flex justify-between items-start mb-1.5">
                        ...
                        <span className={`text-[10px] font-semibold ${isActive ? 'text-white/70' : 'text-outline'}`}>
                      ...
                      <p className={`text-[10px] mt-1 font-medium ${isActive ? 'text-white/65' : 'text-outline'}`}>
                        {course.instructor}
                      </p>
                    </button>
                  );
            ...
            <div className="bg-white border border-outline-variant/30 rounded-xl p-5 shadow-sm">
              ...
              <p className="text-[11px] text-on-surface-variant mb-4 font-medium">
                Styr tilgang og låsing av moduler.
              </p>
              <div className="space-y-2">
                {PROGRESSION_RULES.map(rule => (
                  ...
                  >
                    <div className={`w-3.5 h-3.5 rounded-full border-2 mt-0.5 flex items-center justify-center shrink-0 ${
              ...
              >
                ...
                <div className="p-6 sm:p-8 border-b border-outline-variant/20 bg-gradient-to-r from-white to-primary/[0.025]">
                  {isEditingCourse ? (
                    ...
                    <div className="flex flex-col gap-4">
                      <span className="text-[9px] font-bold uppercase tracking-widest px-3 py-1 bg-primary text-white rounded-full font-mono self-start">
                        Rediger kursinfo
                      </span>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        ...
                        <div className="sm:col-span-2 space-y-1">
                          <label className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-outline">
                            <BookOpen size={11} /> Kurstittel
                          </label>
                          <input
                            value={courseEditDraft.title || ''}
                            onChange={e => setCourseEditDraft(p => ({ ...p, title: e.target.value }))}
                            placeholder="Kurstittel…"
                            className="w-full px-3.5 py-2.5 border-2 border-primary/30 focus:border-primary rounded-lg text-sm font-bold text-primary focus:outline-none shadow-sm font-serif transition-colors"
                            style={{ transform: 'translateZ(0)', backfaceVisibility: 'hidden' }}
                          />
                        </div>
                        {/* Code */}
                        <div className="space-y-1">
                          <label className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-outline">
                            <Hash size={11} /> Kurskode
                          </label>
                          <input
                            value={courseEditDraft.code || ''}
                            onChange={e => setCourseEditDraft(p => ({ ...p, code: e.target.value.toUpperCase() }))}
                            placeholder="f.eks. PROP 101"
                            className="w-full px-3 py-2 border-2 border-outline-variant/40 focus:border-primary rounded-lg text-xs font-bold font-mono text-primary focus:outline-none shadow-sm transition-colors"
                            style={{ transform: 'translateZ(0)', backfaceVisibility: 'hidden' }}
                          />
                        </div>
                        {/* Instructor */}
                        <div className="space-y-1">
                          <label className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-outline">
                            <User size={11} /> Ansvarlig mentor
                          </label>
                          <input
                            value={courseEditDraft.instructor || ''}
                            onChange={e => setCourseEditDraft(p => ({ ...p, instructor: e.target.value }))}
                            placeholder="Mentors fulle navn"
                            className="w-full px-3 py-2 border-2 border-outline-variant/40 focus:border-primary rounded-lg text-xs font-semibold text-primary focus:outline-none shadow-sm transition-colors"
                            style={{ transform: 'translateZ(0)', backfaceVisibility: 'hidden' }}
                          />
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3 pt-1">
                        <button
                          ...
                          className="flex items-center gap-1.5 px-5 py-2.5 bg-primary text-white text-xs font-bold uppercase rounded-lg shadow-md hover:bg-primary/90 transition-all active:scale-95"
                        >
                          <Save size={13} /> Lagre endringer
                        </button>
                        <button
                          onClick={cancelCourseEdit}
                          className="flex items-center gap-1.5 px-4 py-2.5 border border-outline-variant rounded-lg text-xs font-bold uppercase text-on-surface-variant hover:text-primary hover:border-primary transition-all active:scale-95"
                        >
                          <XCircle size={13} /> Avbryt
                        </button>
                      </div>
                    </div>
                  ) : (
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                      ...
                      <div className="flex items-center gap-2.5 shrink-0">
                        <button
                          ...
                          className="py-2 px-4 rounded-lg border border-outline-variant hover:border-primary text-xs font-bold uppercase flex items-center gap-1.5 hover:text-primary transition-all active:scale-95 shadow-sm"
                        >
                          <Pencil size={13} /> Rediger kurs
                        </button>
                        <button
                          onClick={() => navigate('/student/library')}
                          className="py-2 px-4 rounded-lg border border-outline-variant hover:border-primary text-xs font-bold uppercase flex items-center gap-1.5 hover:text-primary transition-all active:scale-95 shadow-sm"
                        >
                          <Eye size={13} /> Studentvisning
                        </button>
                      </div>
                    </div>
                  )}
                ...
                <div className="p-5 sm:p-8 flex flex-col gap-3">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-[10px] font-bold text-outline uppercase tracking-wider bg-slate-50 px-2.5 py-1 rounded-full font-mono">
                      {activeCourse.modules.length} moduler
                    </span>
                  </div>
                ...
                <form onSubmit={handleAddModule} className="border-t border-outline-variant/20 px-5 sm:px-8 py-6 flex flex-col gap-4 bg-slate-50/50">
                  <h3 className="font-serif text-base font-bold text-primary flex items-center gap-2">
                    <PlusCircle size={16} className="text-[#c5a059] shrink-0" />
                    Legg til ny modul
                  </h3>
                  ...
                  <div className="bg-white border border-outline-variant/20 p-3.5 rounded-xl flex gap-2.5 items-start">
                    ...
                    <p className="text-[11px] text-on-surface-variant leading-relaxed font-semibold">
                      <strong>CMS Sanntidsoppdatering:</strong> Ny modul er umiddelbart synlig for studenter i sin studieplan.
                    </p>
                  </div>
                </form>
              </motion.div>
              ...
              <div className="flex items-center justify-between p-5 border-b border-outline-variant/20 bg-[#1B4965] text-white shrink-0">
                <div>
                  <div className="flex items-center gap-2">
                    <MediaIcon size={17} className="text-[#c5a059]" />
                    <h3 className="font-serif text-lg font-bold">Koble Medieressurser</h3>
                  </div>
                  <p className="text-[11px] text-white/65 font-medium mt-0.5">
                    Velg filer å knytte til modulen
                  </p>
                </div>
                <button onClick={closeMediaDrawer} className="p-2 hover:bg-white/10 rounded-lg transition-colors active:scale-90">
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto divide-y divide-outline-variant/10 p-2">
                {filteredMedia.length > 0 ? filteredMedia.map(asset => {
                  ...
                  return (
                    <button
                      key={asset.id}
                      onClick={() => setPendingAssetIds(prev =>
                        prev.includes(asset.id) ? prev.filter(id => id !== asset.id) : [...prev, asset.id]
                      )}
                      className={`w-full text-left flex items-center gap-3.5 p-3.5 rounded-xl transition-all group ${
                    ...
                    >
                      <div className="w-11 h-11 rounded-lg bg-slate-100 flex-shrink-0 overflow-hidden flex items-center justify-center border border-outline-variant/20">
                        {asset.thumbnail
                          ...
                          : <FileIcon type={asset.type} size={18} />
                        }
                      </div>
                      <div className="flex-grow min-w-0">
                      ...
                      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 transition-all ${
                        isSelected ? 'border-primary bg-primary' : 'border-outline-variant/40 group-hover:border-primary/40'
                      }`}>
                ...
                {pendingAssetIds.length > 0 && (
                  <div className="flex items-center gap-2 mb-3.5 p-3 bg-primary/5 border border-primary/15 rounded-lg">
                    ...
                    <p className="text-xs font-bold text-primary">
                      {pendingAssetIds.length} ressurs{pendingAssetIds.length !== 1 ? 'er' : ''} valgt
                    </p>
                  </div>
                )}
