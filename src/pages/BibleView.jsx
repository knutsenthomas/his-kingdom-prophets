...
import { 
  ...
  Trash2, Save, Loader2, ChevronDown, Maximize2, Minimize2, Type, Edit3, Users, History
} from 'lucide-react';
...
export default function BibleView() {
  ...
  const { showToast, sendAssistantMessage, user, setAssistantContext, isAdminEditing, setIsAdminEditing, language } = useApp();
  ...
  const [activeViewTab, setActiveViewTab] = useState('bible'); // bible, characters, timeline
  ...
  return (
    <main className="flex-grow p-4 md:p-10 space-y-8" ref={topRef}>
      ...
      <section className="space-y-3">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1">
            <h1 className="font-serif text-3xl md:text-4xl font-bold text-primary flex items-center gap-2.5">
              {activeViewTab === 'bible' && <BookOpen className="text-primary" size={28} />}
              {activeViewTab === 'characters' && <Users className="text-primary" size={28} />}
              {activeViewTab === 'timeline' && <History className="text-primary" size={28} />}
              {activeViewTab === 'bible' && "Den Hellige Skrift"}
              {activeViewTab === 'characters' && "Bibelske personer"}
              {activeViewTab === 'timeline' && "Bibelsk tidslinje"}
            </h1>
            <p className="text-sm text-on-surface-variant max-w-2xl leading-relaxed">
              {activeViewTab === 'bible' && "Utforsk, les og studer Guds ord i ulike oversettelser. Klikk på vers for å dele dem med bønnefellesskapet, kopiere eller be HKM Assistenten om en dypere teologisk utredning."}
              {activeViewTab === 'characters' && "Utforsk biografiene, de sentrale bibelske skriftene, og den teologiske betydningen av Bibelens mest sentrale skikkelser."}
              {activeViewTab === 'timeline' && "Følg den røde tråden gjennom Bibelens historiske epoker og få oversikt over imperiene som formet den bibelske historien."}
            </p>
          </div>

          {/* Quick search input (only shown on bible tab) */}
          {activeViewTab === 'bible' && (
        ...
        {/* Tab Switcher */}
        <div className="flex border-b border-slate-200 overflow-x-auto whitespace-nowrap scrollbar-none h-14 items-center gap-6 mt-4">
          {[
            { id: 'bible', label: 'Bibel', icon: BookOpen },
            { id: 'characters', label: 'Bibelske personer', icon: Users },
            { id: 'timeline', label: 'Bibelsk tidslinje', icon: History }
          ].map(tab => {
            const Icon = tab.icon;
            ...
            {(returnOriginTab || previousReference) && (
              <div className="mb-4 flex animate-fade-in justify-start select-none">
                {returnOriginTab ? (
                  <button
                    onClick={() => {
                      setActiveViewTab(returnOriginTab);
                      setReturnOriginTab(null);
                    }}
                    className="flex items-center gap-1.5 px-4 py-2 bg-gradient-to-r from-burnt-orange to-[#bd4f2a] hover:from-[#bd4f2a] hover:to-[#a03d1e] text-white shadow-md shadow-burnt-orange/10 rounded-full text-xs font-extrabold transition-all duration-200 hover:-translate-y-0.5 active:translate-y-0 active:scale-[0.97] cursor-pointer"
                  >
                    <ArrowLeft size={12} className="stroke-[3]" />
                    <span>
                      {returnOriginTab === 'characters' ? 'Tilbake til personer' : 'Tilbake til tidslinje'}
                    </span>
                  </button>
                ) : (
                  <button
